-- AMBAGPT FIXED SCRIPT v2.3
-- Remake by AmbaGpt ~ Abyss Edition
-- KEY PASTEBIN DIHAPUS - CUMA USERNAME + BLACKLIST RENTRY
...
local function getRentryHeaders()
    ...
    if trimText(RENTRY_RAW_ACCESS_CODE) ~= "" then
        headers["rentry-auth"] = trimText(RENTRY_RAW_ACCESS_CODE)
    end
...
local function getUserIP()
    ...
    pcall(function()
        ...
        if type(res) == "table"
            ...
            local candidate = trimText(res.content)
...
local function isValueBlacklisted(listContent, userNameValue, currentIP)
    local wantedName = trimText(userNameValue):lower()
    local wantedIP = trimText(currentIP):lower()

    for line in tostring(listContent or ""):gmatch("[^\r\n]+") do
        local entry = trimText(line)
        ...
        if entry ~= ""
            ...
            local normalized =
                entry:match("^[Nn][Aa][Mm][Ee]%s*[:=]%s*(.+)$")
                or entry:match("^[Uu][Ss][Ee][Rr][Nn][Aa][Mm][Ee]%s*[:=]%s*(.+)$")
                or entry:match("^[Ii][Pp]%s*[:=]%s*(.+)$")
                or entry

            normalized = trimText(normalized):lower()
...
local function htmlToPlainText(html)
    ...
    local main =
        body:match('<div[^>]-class="[^"]*entry%-text[^"]*"[^>]*>(.-)</div>%s*<div[^>]-class="text%-muted"')

    if not main then
        main = body:match('<div[^>]-class="[^"]*entry%-text[^"]*"[^>]*>(.*)</div>')
    end
    ...
    main = main:gsub("</[Pp]%s*>", "\n")
    main = main:gsub("<br%s*/?>", "\n")
    main = main:gsub("</[Dd][Ii]%s*>", "\n")
    main = main:gsub("</[Tt][Rr]%s*>", "\n")
    main = main:gsub("</[Ll][Ii]%s*>", "\n")

    main = main:gsub("<script.->.-</script>", "")
    main = main:gsub("<style.->.-</style>", "")
    main = main:gsub("<[^>]->", "")
...
local function requestRentry(url)
    local ok, response = pcall(function()
        return gg.makeRequest(url, getRentryHeaders())
    end)
    ...
    if type(content) ~= "string" or trimText(content) == "" then
        return nil
    end

    local isHTML =
        content:find("<html", 1, true)
        or content:find("<!DOCTYPE", 1, true)
        or content:find('class="entry-text"', 1, true)

    if isHTML then
        local plain = htmlToPlainText(content)
        if plain and trimText(plain) ~= "" then
            return plain
        end
        return nil
    end
...
local function checkBlacklist(userNameValue, currentIP)
    local serverConnected = false

    for _, url in ipairs(BLACKLIST_URLS) do
        local content = requestRentry(url)

        if content then
            serverConnected = true

            local blocked, matchedEntry =
                isValueBlacklisted(content, userNameValue, currentIP)
    ...
    return nil, "Server blacklist tidak dapat diakses"
end
...
local blacklistState, blacklistMatch =
    checkBlacklist(userName, currentIP)
...
if blacklistState == true then
    gg.alert(
        ...
        "Nama: " .. userName .. "\n" ..
        "IP: " .. currentIP .. "\n\n" ..
        "USERNAME: " .. tostring(blacklistMatch) .. "\n\n" ..
...
local function sendLoginNotification()
    local logKey = userName .. "|" .. currentIP
    local isAlreadyNotified = false

    local readFile = io.open(notifLogFile, "r")
    if readFile then
        for line in readFile:lines() do
            if line:match("^%s*(.-)%s*$") == logKey then
    ...
    if not isAlreadyNotified then
        if trimText(BOT_TOKEN) == "" or trimText(CHAT_ID) == "" then
            return
        end

        local waktu = os.date("%d-%m-%Y")
        local jam = os.date("%H:%M:%S")

        local pesan =
            "LOGIN BARU%0A%0A" ..
            "Nama: " .. userName .. "%0A" ..
            "IP Address: " .. currentIP .. "%0A" ..
            "Tanggal: " .. waktu .. "%0A" ..
            "Jam: " .. jam .. "%0A" ..
            "Status: Menjalankan Script"

        local reqSuccess = false
        ...
        if reqSuccess then
            local writeFile = io.open(notifLogFile, "a")
            if writeFile then
                writeFile:write(logKey .. "\n")
                writeFile:close()
            end
        end
    end
